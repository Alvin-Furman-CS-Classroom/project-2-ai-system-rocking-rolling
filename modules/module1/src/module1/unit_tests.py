...
def load_track(low_file_name: str, high_file_name: str):
    ...
    if not low_file.exists() or not high_file.exists():
        raise FileNotFoundError(
            f"Could not find test files: {low_file_name}, {high_file_name}"
        )
...
def print_components(result) -> None:
    """Print all compatibility component scores."""
    ...
    print(f"  Genre: {result.genre_compatibility:.1%}")
    print(f"  Tags: {result.tag_compatibility:.1%}")
    print(f"  Popularity: {result.popularity_compatibility:.1%}")
    print(f"  Artist: {result.artist_compatibility:.1%}")
    print(f"  Era: {result.era_compatibility:.1%}")
    print(f"  MB Genre: {result.mb_genre_compatibility:.1%}")
...
def get_track_info(high_file_name: str) -> dict:
    ...
    return {
        ...
        "album": metadata.get("album", ["Unknown"])[0]
...
def test_same_piece_twice():
    ...
    assert result.probability > 0.7, (
        f"Expected >0.7 for same piece, got {result.probability:.1%}"
    )
