...
The system has four modules. Module~1 establishes scoring; Module~2 uses it for path finding; Module~3 refines paths and explains; Module~4 trains a mood classifier which can help filter the song picker to fit a particular mood.
...
Key uses Pearson correlation on Krumhansl-Kessler probe-tone profiles \cite{krumhansl1990}; tempo uses Gaussian decay with 2:1 ratio handling \cite{drake1993}; timbre is the Bhattacharyya coefficient on MFCC Gaussians \cite{aucouturier2002,berenzweig2004}; energy, loudness, popularity, and era use Gaussian decay over their respective AcousticBrainz fields \cite{korzeniowski2021,schweiger2025}; genre and tags use cosine/Jaccard similarity \cite{alonso2022}. Each score is asserted as a probabilistic fact in ProbLog \cite{deraedt2007}, which combines weighted terms into the overall probability.

\subsection{Module 2: Path Finding and Data Integration}

Module~2 takes a source MBID, destination MBID, target length, and beam width, and returns a \texttt{PlaylistPath} of waypoint MBIDs and accumulated cost. It runs bidirectional beam search with an A*~\cite{hart1968} heuristic: $f(n) = g(n) + h(n)$, where $g$ is accumulated cost and $h(n) = 1 - \text{compat}(n, \text{goal})$ from Module~1. Two frontiers expand from source and destination and terminate when they meet or the target length is reached; beam width $B = 10$ retains only the top $B$ states per round.
