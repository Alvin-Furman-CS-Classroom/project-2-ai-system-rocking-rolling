  ...
  return (
    <div className={`slide ${isActive ? 'active' : ''}`}>
      ...
      <div style={{
        ...
        {/* Left: track list */}
        <div style={{ width: 590, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Compatibility arc */}
          <div className="card" style={{ padding: '14px 16px', flex: '0 0 auto' }}>
          ...
          <div style={{ flex: 1, overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              {playlist.constraints.map(c => (
                <div key={c.name} className="card" style={{ padding: '10px 14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 13, fontWeight: 'bold' }}>{formatConstraintName(c.name)}</span>
                    <span style={{
                      fontSize: 14,
                      fontWeight: 'bold',
                  ...
                  <p style={{ fontSize: 11, color: T.MUTED, marginTop: 4, textAlign: 'right' }}>
...
function StatPill({ label, value }: { label: string; value: string }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 15, fontWeight: 'bold', color: T.ORANGE }}>{value}</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <span style={{
        ...
        fontWeight: 'bold', color: T.ORANGE,
      ...
      {/* Key */}
      {track.key && (
        <span style={{ fontSize: 12, color: T.MUTED, flexShrink: 0, width: 48 }}>
      ...
      {/* BPM */}
      {track.bpm && (
        <span style={{ fontSize: 12, color: T.MUTED, flexShrink: 0, width: 54, textAlign: 'right' }}>
          {track.bpm} BPM
        </span>
      )}
    </div>
...
function CompatibilityArc({ transitions }: { transitions: Transition[] }) {
  ...
  const points = transitions.map((t, i) => ({ x: xScale(i), y: yScale(t.probability), p: t.probability }));
    ...
    <svg width={W} height={H} style={{ display: 'block' }}>
      ...
      {points.map((p, i) => (
        <text
          key={i}
          x={p.x} y={H - 4}
          textAnchor="middle" fontSize={10} fill={T.MUTED}
        >
          {i + 1}→{i + 2}
        </text>
      ))}
      ...
      {points.map((p, i) => (
        <text
          ...
          fontWeight="bold" fill={T.ORANGE_D}
        >
          {Math.round(p.p * 100)}%
        </text>
      ))}
